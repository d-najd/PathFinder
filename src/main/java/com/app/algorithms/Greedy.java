...
public class Greedy implements ISearchAlgorithm {
	 ...
	 * the algorithm looks at all the neighbors, and it calculates the distance from
	 * itself to the finish, it does this
	 * by taking his x and y pos and subtracting it from the end's x and y pos, for
	 * example if the start is located at
	 * 5,10 and the end is located at 2,15 it's going do the following equation
	 * ((5-2)+(10-15))=3+5=8 distance away from
	 * the end, NOTE it shouldn't support negative numbers so any negative number
	 * like in the case with 10-15 will be
	 * converted to positive in this case 5, and then the algorithm does that for
	 * each of the neighbors and takes the one
	 * which is the closest to the end (NOTE it stores the data of the previous
	 * neighbors) after it moves it checks if it
	 * has any new neighbors calculates how far away they are from the end and takes
	 * the next closest and so on till it
	 * reaches the end
	 */
			...
			for (int i = 0; i < 4; i++) {
				...
				if (checkedPiece != null) {
					if (checkedPiece.getType() == Piece.Type.Empty) {
						...
						// prevents adding the same piece to the list thus preventing a memory leak and
